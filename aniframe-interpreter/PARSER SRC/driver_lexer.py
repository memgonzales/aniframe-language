...
def should_display_token_text(complete_display_mode, token_type):
    return complete_display_mode or token_type in ['IDENTIFIER', 'DATA_TYPE'] or token_type.endswith('LITERAL')
...
def tokenize(source_code, complete_display_mode):
    ...
    for token in tokens:
        ...
        if should_display_token_text(complete_display_mode, lexer.symbolicNames[token.type]):
