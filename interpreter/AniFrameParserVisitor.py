...
def assign_member_value(val, idx, expr, assignment, ctx):
    ...
    if (
        isinstance(VARIABLES[val]["value"], list)
        and idx >= 0
        and idx <= len(VARIABLES[val]["value"])
    ):
        match assignment:
            case None:
                if VARIABLES[val]["value"][idx]["data_type"] == dtype:
                    ...
                    raiseError(
                        ctx,
                        TypeError,
                        f" {dtype} to {VARIABLES[val]['value'][idx]['data_type']} assignment is not supported",
                    )
            case "+":
                match VARIABLES[val]["value"][idx]["data_type"], dtype:
                    ...
                    case "Text", "Text":
                        VARIABLES[val]["value"][idx]["value"] += expr["value"]
                    case "Color", "Color":
                        VARIABLES[val]["value"][idx]["value"][0] = int(
                            (
                                VARIABLES[val]["value"][idx]["value"][0]
                                + expr["value"][0]
                            )
                            / 2
                        )
                        VARIABLES[val]["value"][idx]["value"][1] = int(
                            (
                                VARIABLES[val]["value"][idx]["value"][1]
                                + expr["value"][1]
                            )
                            / 2
                        )
                        VARIABLES[val]["value"][idx]["value"][2] = int(
                            (
                                VARIABLES[val]["value"][idx]["value"][2]
                                + expr["value"][2]
                            )
                            / 2
                        )
                    case "Coord", "Coord":
                        VARIABLES[val]["value"][idx]["value"] = (
                    ...
                    case _:
                        ...
                        raiseError(
                            ctx,
                            TypeError,
                            f"{assignment} operator between {VARIABLES[val]['value'][idx]['data_type']} and {dtype} is not supported",
                        )
            case "-":
                match VARIABLES[val]["value"][idx]["data_type"], dtype:
                    case "Number", "Number":
            ...
            case "%":
                match VARIABLES[val]["value"][idx]["data_type"], dtype:
                    case "Number", "Number":
                        VARIABLES[val]["value"][idx]["value"] = int(
                            VARIABLES[val]["value"][idx]["value"]
                        ) % int(expr["value"])
                    case _:
                        # throw error
...
def check_parameters(name, params, ctx):
    match name:
            ...
            if check:
                ...
                if params[0]["data_type"] == "Object":
                    try:
                        ...
                        if iden == None:
                            iden = mapping.convert_object_expr_to_p5(
        ...
        case "fill":
            check = True if len(params) == 1 else False
            if check and (
                len(params[0]["value"]) == 3 and params[0]["data_type"] == "Color"
            ):
                params = params[0]["value"]
                return params
            if check and (
                is_hexColor(params[0]["value"]) or is_rgb(params[0]["value"])
            ):
...
def check_user_params(name, params, ctx):
    ...
    if check:
        for i in range(len(params)):
            if (
                func_params[i]["data_type"] == "Color"
                and params[i]["data_type"] == "Text"
            ):
                ...
                if color:
                    ...
                    raiseError(
                        ctx,
                        ValueError,
                        f"Invalid parameters for {name}. Parameter {i+1} must be {func_params[i]['data_type']}",
                    )
...
class AniFrameParserVisitor(ParseTreeVisitor):
    ...
    def visitDraw_statement(self, ctx: AniFrameParser.Draw_statementContext):
        ...
        if VARIABLES[var_name]["data_type"] != "Object":
            raiseError(
                ctx,
                TypeError,
                f"{VARIABLES[var_name]['data_type']} does not have method draw",
            )
            ...
            if VARIABLES[var_name]["data_type"] == "List":
                ...
                match func_name:
                    case "add":
                        ...
                        if type(result[0]) != str:
                            raiseError(
                                ctx,
                                ValueError,
                                f"Cannot add {result[0]['value']} to object",
                            )
                    ...
                    case "draw":
                        mapping.draw(var_name, result[0], result[1])
                    case "turn":
                        mapping.convert_rotate_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "turnC":
                        mapping.convert_rotateC_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "turnCC":
                        mapping.convert_rotateCC_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "move":
                        mapping.convert_move_to_p5(
                            var_name, result[0], result[1], result[2], result[3]
                        )
                    case "moveX":
                        mapping.convert_moveX_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "moveY":
                        mapping.convert_moveY_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "shear":
                        mapping.convert_shear_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "shearX":
                        mapping.convert_shearX_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "shearY":
                        mapping.convert_shearY_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "write":
                        mapping.convert_write_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "resize":
                        mapping.convert_resize_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "resizeX":
                        mapping.convert_resizeX_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "resizeY":
                        mapping.convert_resizeY_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "fill":
                        mapping.convert_fill_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case "stroke":
                        mapping.convert_stroke_to_p5(
                            var_name, result[0], result[1], result[2]
                        )
                    case _:
                        raiseError(
                            ctx, ValueError, f"List does not have method {func_name}"
                        )
            else:
                raiseError(
                    ctx,
                    TypeError,
                    f"{VARIABLES[var_name]['data_type']} method calls are not supported",
                )
            ...
            match func_name:
                ...
                case "info":
                    if result[0]["data_type"] == "Object":
                        print(
                            mapping.CLASSES[
                                mapping.clean_identifier(result[0]["identifier"])
                            ]
                        )
                    elif result[0]["data_type"] == "Color":
                        print(
                            f"rgb{result[0]['value'][0],result[0]['value'][1],result[0]['value'][2]}"
                        )
                    else:
                        res = result[0]["value"]
                        if res == "_":
                ...
                case func_name if func_name in user_function_names:
                    ...
                    function_ctr += 1
                    val = self.visitFunction_declaration_definition(
                        context, params, return_value
                    )
                    function_ctr -= 1
                    return
                case _:
        ...
        if ctx.getChildCount() == 3:
            ...
            if expr1["data_type"] == "_":
                ...
                raiseError(
                    ctx,
                    ValueError,
                    f'Identifier {expr1["identifier"]} has not been initialized',
                )
            if expr2["data_type"] == "_":
                ...
                raiseError(
                    ctx,
                    ValueError,
                    f'Identifier {expr2["identifier"]} has not been initialized',
                )
            match operator:
                case "+":
                    match expr1["data_type"], expr2["data_type"]:
                        case "Object", "Object":
                            iden = mapping.convert_object_add_to_p5(
                                expr1["identifier"], expr2["identifier"]
                            )
                            return {
                                "value": mapping.CLASSES[iden],
                                "data_type": "Object",
                                "identifier": iden,
                            }
                        case "Number", "Number":
                        ...
                        case "Color", "Color":
                            res = [
                                int((expr1["value"][0] + expr2["value"][0]) / 2),
                                int((expr1["value"][1] + expr2["value"][1]) / 2),
                                int((expr1["value"][2] + expr2["value"][2]) / 2),
                            ]
                            return {"value": res, "data_type": "Color"}
                        case "Color", "Text":
                            iscolor = is_rgb(expr2["value"]) or is_hexColor(
                                expr2["value"]
                            )
                            if iscolor:
                                ...
                                raiseError(
                                    ctx,
                                    TypeError,
                                    f"{operator} operator between {expr1['data_type']} and {expr2['data_type']} is not supported",
                                )
                            res = [
                                int((expr1["value"][0] + colors[0]) / 2),
                                int((expr1["value"][1] + colors[1]) / 2),
                                int((expr1["value"][2] + colors[2]) / 2),
                            ]
                            return {"value": res, "data_type": "Color"}
                        case "Text", "Color":
                            iscolor = is_rgb(expr1["value"]) or is_hexColor(
                                expr1["value"]
                            )
                            if iscolor:
                            ...
                            res = [
                                int((expr2["value"][0] + colors[0]) / 2),
                                int((expr2["value"][1] + colors[1]) / 2),
                                int((expr2["value"][2] + colors[2]) / 2),
                            ]
                            return {"value": res, "data_type": "Color"}
                        case "Coord", "Coord":
                            res = (
                                expr1["value"][0] + expr2["value"][0],
                                expr1["value"][1] + expr2["value"][1],
                            )
                ...
                case "-":
                    match expr1["data_type"], expr2["data_type"]:
                        ...
                        case "Text", "Color":
                            ...
                            res = [
                                int((colors[0] + colors2[0]) / 2),
                                int((colors[1] + colors2[1]) / 2),
                                int((colors[1] + colors2[1]) / 2),
                            ]
                            return {"value": res, "data_type": "Color"}
                        case "Coord", "Coord":
                ...
                case "*":
                    match expr1["data_type"], expr2["data_type"]:
                        ...
                        case "Coord", "Number":
                            res = (
                                expr1["value"][0] * expr2["value"],
                                expr1["value"][1] * expr2["value"],
                            )
                            return {"value": res, "data_type": "Coord"}
                        case "Number", "Coord":
                            res = (
                                expr1["value"] * expr2["value"][0],
                                expr1["value"] * expr2["value"][1],
                            )
                            return {"value": res, "data_type": "Coord"}
                        case "Coord", "Coord":
                            res = (
                    ...
                    match expr1["data_type"], expr2["data_type"]:
                        ...
                        case "Coord", "Coord":
                            res = (
                                ...
                                or (
                                    expr1["value"][0] == expr2["value"][0]
                                    and expr1["value"][1] < expr2["value"]["1"]
                                )
                                else 0
                            )
                            return {"value": res, "data_type": "Number"}
                ...
                case _:
                    raiseError(
                        ctx,
                        ValueError,
                        f"Expression operator {operator} does not exist",
                    )
            ...
            if opr == "!":
                ...
                if expr["data_type"] != "Number":
                    ...
                    raiseError(
                        ctx,
                        TypeError,
                        f'Unary operator {opr} cannot be evaluated with {expr["data_type"]}',
                    )
            ...
            if isinstance(val, dict):
                ...
                match func_name:
                    case "length":
                        ...
                        if check:
                            return {
                                "value": len(params[0]["value"]),
                                "data_type": "Number",
                            }
                        else:
                            raiseError(
                                ctx, ValueError, f"Wrong parameters for {func_name}"
                            )
                    case "get_input":
                        check = True if len(params) == 1 else False
                        if check:
                        ...
                        if check:
                            try:
                                ...
                                raiseError(
                                    ctx,
                                    ValueError,
                                    f'File {params[0]["value"]} does not exist',
                                )
                    ...
                    case "asin":
                        ...
                        if check:
                            return {
                                "value": math.asin(params[0]["value"]),
                                "data_type": "Number",
                            }
                        else:
                            # THROW ERROR
                    ...
                    case "acos":
                        ...
                        if check:
                            return {
                                "value": math.acos(params[0]["value"]),
                                "data_type": "Number",
                            }
                        else:
                            # THROW ERROR
                    ...
                    case "atan":
                        ...
                        if check:
                            return {
                                "value": math.atan(params[0]["value"]),
                                "data_type": "Number",
                            }
                        else:
                            # THROW ERROR
                    ...
                    case "to_rad":
                        ...
                        if check:
                            return {
                                "value": math.radians(params[0]["value"]),
                                "data_type": "Number",
                            }
                        else:
                            # THROW ERROR
                    ...
                    case "to_deg":
                        ...
                        if check:
                            return {
                                "value": math.degrees(params[0]["value"]),
                                "data_type": "Number",
                            }
                        else:
                            # THROW ERROR
                        ...
                        if check:
                            return {
                                "value": random.randint(
                                    int(params[0]["value"]), int(params[1]["value"])
                                ),
                                "data_type": "Number",
                            }
                        else:
                    ...
                    case "write":
                        ...
                        if check:
                            if params[0]["data_type"] != "Text":
                                check = False
                            if (
                                params[1]["data_type"] != "Number"
                                or params[2]["data_type"] != "Number"
                            ):
                                check = False
                        if check:
                            params = [
                    ...
                    case func_name if func_name in user_function_names:
                        ...
                        if val == None:
                            raiseError(
                                ctx, ValueError, f"{func_name} has no return value"
                            )
                        return val
                    case _:
                        raiseError(
                            ctx, ValueError, f"{func_name} is not a valid function"
                        )
    ...
    def visitMember(self, ctx: AniFrameParser.MemberContext):
        ...
        if (
            expr["data_type"] != "Number"
            or expr_2["data_type"] != "Number"
            and expr_2["data_type"] != None
        ):
            # throw error
            raiseError(ctx, TypeError, f"Invalid index data type")
        idx = int(expr["value"])
        if (
            VARIABLES[iden]["data_type"] == "List"
            or VARIABLES[iden]["data_type"] == "Coord"
        ):
                ...
                if children > 4:
                    if VARIABLES[iden]["value"][idx]["data_type"] != "Coord":
                        ...
                        raiseError(
                            ctx,
                            TypeError,
                            f"{VARIABLES[iden]['value'][idx]['data_type']} is not iterable",
                        )
    ...
    def visitFunction_call(self, ctx: AniFrameParser.Function_callContext):
        ...
        if (
            func_name not in built_in_func_names
            and func_name not in user_function_names
        ):
    ...
    def visitVariable_declaration(
        self, ctx: AniFrameParser.Variable_declarationContext
    ):
        ...
        if (
            VARIABLES[iden]["data_type"] == "_"
            or "data_type" not in VARIABLES[iden]
            or VARIABLES[iden]["data_type"] == dtype
        ):
            ...
            raiseError(
                ctx,
                TypeError,
                f'{iden} has already been declared with data type {VARIABLES[iden]["data_type"]}',
            )

    # Visit a parse tree produced by AniFrameParser#assignment_statement.
    def visitAssignment_statement(
        self, ctx: AniFrameParser.Assignment_statementContext
    ):
        idxstart = 0
        constant = False
        ...
        if ":" in val:
            ...
            VARIABLES[val]["data_type"] = (
                "_"
                if "data_type" not in VARIABLES[val]
                else VARIABLES[val]["data_type"]
            )
        ...
        if expr["data_type"] == "Object":
            try:
                ...
                if object_expr_iden == None:
                    object_expr_iden = mapping.convert_object_expr_to_p5(
        ...
        if len(assignment) == 1:
            if "constant" in VARIABLES[val]:
                ...
                raiseError(
                    ctx, TypeError, f"Cannot change value of const variable {val}"
                )
                ...
                if color:
                    ...
                    raiseError(
                        ctx,
                        TypeError,
                        f"Invalid assignment statement. Text can not be converted to color",
                    )
            else:
                if (
                    VARIABLES[val]["data_type"] == expr["data_type"]
                    or VARIABLES[val]["data_type"] == "_"
                ):
                    ...
                    if (
                        VARIABLES[val]["data_type"] == "Object"
                        and expr["data_type"] == "Object"
                    ):
                    ...
                    raiseError(
                        ctx,
                        TypeError,
                        f"Invalid assignment statement. Data type mismatch",
                    )
            ...
            if VARIABLES[val]["data_type"] == "_" or VARIABLES[val]["value"] == "_":
                # THROW ERROR
                raiseError(
                    ctx, ValueError, f"Identifier {val} has not been initialized"
                )
            match assignment[0]:
                case "+":
                    match VARIABLES[val]["data_type"], expr["data_type"]:
                        ...
                        case "Text", "Text":
                            ...
                            return val, VARIABLES[val]
                        case "Color", "Color":
                            VARIABLES[val]["value"][0] = int(
                                (VARIABLES[val]["value"][0] + expr["value"][0]) / 2
                            )
                            VARIABLES[val]["value"][1] = int(
                                (VARIABLES[val]["value"][1] + expr["value"][1]) / 2
                            )
                            VARIABLES[val]["value"][2] = int(
                                (VARIABLES[val]["value"][2] + expr["value"][2]) / 2
                            )
                        case "Color", "Text":
                            iscolor = is_rgb(expr["value"]) or is_hexColor(
                                expr["value"]
                            )
                            if iscolor:
                                colors = hexrgb_to_colors(expr["value"])
                                VARIABLES[val]["value"][0] = int(
                                    (VARIABLES[val]["value"][0] + colors[0]) / 2
                                )
                                VARIABLES[val]["value"][1] = int(
                                    (VARIABLES[val]["value"][1] + colors[1]) / 2
                                )
                                VARIABLES[val]["value"][2] = int(
                                    (VARIABLES[val]["value"][2] + colors[2]) / 2
                                )
                            else:
                                raiseError(
                                    ctx,
                                    ValueError,
                                    f"{expr['value']} can not be treated as color",
                                )
                        case "Coord", "Coord":
                            VARIABLES[val]["value"] = (
                        ...
                        case _:
                            ...
                            raiseError(
                                ctx,
                                TypeError,
                                f"{assignment[0]} operator between {VARIABLES[val]['data_type']} and {expr['data_type']} is not supported",
                            )
                case "-":
                    match VARIABLES[val]["data_type"], expr["data_type"]:
                        case "Number", "Number":
                ...
                case _:
                    raiseError(
                        ctx,
                        TypeError,
                        f"{assignment[0]}= compound operator is not supported",
                    )
        else:
            # THROW ERROR
            ...
            match configurable:
                ...
                case "CANVAS_WIDTH":
                    if VARIABLES[configurable]["value"] >= 7500:
                        raiseError(
                            ctx, ValueError, f"{configurable} can not exceed 7500"
                        )
    ...
    def visitFlow_control_statement(
        self, ctx: AniFrameParser.Flow_control_statementContext
    ):
        return self.visitChildren(ctx)
    ...
    def visitFor_line(self, ctx: AniFrameParser.For_lineContext, ctr, first=False):
        ...
        if iden2 != None and expr["value"][ctr]["data_type"] != "Coord":
            raiseError(
                ctx, TypeError, f"{expr['value'][ctr]['data_type']} is not iterable"
            )
    ...
    def visitRepeat_line(self, ctx: AniFrameParser.Repeat_lineContext):
        ...
        if expr["data_type"] != "Number":
            ...
            raiseError(
                ctx,
                TypeError,
                f'Repeat value must be Number instead of {expr["data_type"]}',
            )
        else:
            return expr["value"]

    # Visit a parse tree produced by AniFrameParser#function_declaration_definition.
    def visitFunction_declaration_definition(
        self,
        ctx: AniFrameParser.Function_declaration_definitionContext,
        params=None,
        retval=None,
    ):
        global VARIABLES
    ...
    def visitFunction_declaration(
        self, ctx: AniFrameParser.Function_declarationContext, parent=None
    ):
        ...
        if func_name not in user_function_names:
            user_function_names.append(func_name)
        retval = None
        txt = ctx.getText()
        if "returns" in txt:
    ...
    def visitReturn_value_data_types(
        self, ctx: AniFrameParser.Return_value_data_typesContext
    ):
        return self.visitChildren(ctx)

    # Visit a parse tree produced by AniFrameParser#return_value_data_type.
    def visitReturn_value_data_type(
        self, ctx: AniFrameParser.Return_value_data_typeContext
    ):
        return self.visitChildren(ctx)
    ...
    def visitConditional_with_break(
        self, ctx: AniFrameParser.Conditional_with_breakContext
    ):
        children = ctx.getChildCount()
    ...
    def visitIf_statement_with_break(
        self, ctx: AniFrameParser.If_statement_with_breakContext
    ):
        if self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == False:
    ...
    def visitElse_if_statement_with_break(
        self, ctx: AniFrameParser.Else_if_statement_with_breakContext
    ):
        if self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == False:
    ...
    def visitElse_statement_with_break(
        self, ctx: AniFrameParser.Else_statement_with_breakContext
    ):
        val = self.visit(ctx.getChild(1))
        if val == False:
            return False
    ...
    def visitConditional_block_with_break(
        self, ctx: AniFrameParser.Conditional_block_with_breakContext
    ):
        children = ctx.getChildCount()
    ...
    def visitConditional_with_return(
        self, ctx: AniFrameParser.Conditional_with_returnContext
    ):
        children = ctx.getChildCount()
    ...
    def visitIf_statement_with_return(
        self, ctx: AniFrameParser.If_statement_with_returnContext
    ):
        if self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == None:
    ...
    def visitElse_if_statement_with_return(
        self, ctx: AniFrameParser.Else_if_statement_with_returnContext
    ):
        if self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == None:
    ...
    def visitElse_statement_with_return(
        self, ctx: AniFrameParser.Else_statement_with_returnContext
    ):
        val = self.visit(ctx.getChild(1))
        if val == None:
            return True
    ...
    def visitConditional_block_with_return(
        self, ctx: AniFrameParser.Conditional_block_with_returnContext
    ):
        children = ctx.getChildCount()
    ...
    def visitConditional_with_break_return(
        self, ctx: AniFrameParser.Conditional_with_break_returnContext
    ):
        children = ctx.getChildCount()
    ...
    def visitIf_statement_with_break_return(
        self, ctx: AniFrameParser.If_statement_with_break_returnContext
    ):
        if self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == None:
    ...
    def visitElse_if_statement_with_break_return(
        self, ctx: AniFrameParser.Else_if_statement_with_break_returnContext
    ):
        if self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == None:
    ...
    def visitElse_statement_with_break_return(
        self, ctx: AniFrameParser.Else_statement_with_break_returnContext
    ):
        val = self.visit(ctx.getChild(1))
        if val == None:
            return True
    ...
    def visitConditional_block_with_break_return(
        self, ctx: AniFrameParser.Conditional_block_with_break_returnContext
    ):
        children = ctx.getChildCount()
        for i in range(2, children):
            val = self.visit(ctx.getChild(i))
    ...
    def visitFor_loop_with_return(
        self, ctx: AniFrameParser.For_loop_with_returnContext
    ):
        ctr = 0
        storage, idens, expr = self.visitFor_line(ctx.getChild(0), ctr, first=True)
        end = len(expr["value"])
    ...
    def visitWhile_loop_with_return(
        self, ctx: AniFrameParser.While_loop_with_returnContext
    ):
        while self.visit(ctx.getChild(0)):
            val = self.visit(ctx.getChild(1))
            if val == False:
    ...
    def visitRepeat_loop_with_return(
        self, ctx: AniFrameParser.Repeat_loop_with_returnContext
    ):
        value = self.visit(ctx.getChild(0))
        for i in range(int(value)):
            val = self.visit(ctx.getChild(1))
    ...
    def visitLoop_block_with_return(
        self, ctx: AniFrameParser.Loop_block_with_returnContext
    ):
        children = ctx.getChildCount()
        for i in range(2, children - 1):
            val = self.visit(ctx.getChild(i))
    ...
    def visitFunction_block(
        self, ctx: AniFrameParser.Function_blockContext, params, return_type
    ):
        for param in params:
        ...
        if return_type == None:
            if value == None:
                ...
                raiseError(
                    ctx,
                    ValueError,
                    f"Value was returned but no return type was declared in the function",
                )
        ...
        if value["data_type"] == "Text" and return_type == "Color":
            ...
            if color:
                colors = hexrgb_to_colors(value["value"])
                return {"value": colors, "data_type": "Color"}
            else:
                raiseError(
                    ctx,
                    ValueError,
                    f"Return value data type does not match declared type",
                )
        if value["data_type"] == return_type:
            return value
        else:
            raiseError(
                ctx, ValueError, f"Return value data type does not match declared type"
            )
    ...
    def visitBinary_logical_operator(
        self, ctx: AniFrameParser.Binary_logical_operatorContext
    ):
        return ctx.getText()
    ...
    def visitCompound_assignment_operator(
        self, ctx: AniFrameParser.Compound_assignment_operatorContext
    ):
        return ctx.getText()
    ...
    def visitAtom(self, ctx: AniFrameParser.AtomContext):
        ...
        if isinstance(val, dict):
            ...
            if atom_text == "FRAME":
                ...
                VARIABLES[atom_text]["data_type"] = (
                    "_"
                    if VARIABLES[atom_text].get("data_type") == None
                    else VARIABLES[atom_text]["data_type"]
                )
