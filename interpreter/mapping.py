...
def convert_object_to_p5(name):
    ...
    CLASSES[name]["display()"] = [
        OrderedDict([("fill", None), ("stroke", None), ("shape", None)])
    ]
...
def convert_curve_to_p5(name, x1, y1, x2, y2, x3, y3, x4, y4):
    convert_shape_to_p5(
        clean_identifier(name), f"curve({x1}, {y1}, {x2}, {y2}, {x3}, {y3}, {x4}, {y4})"
    )
...
def convert_triangle_to_p5(name, x1, y1, x2, y2, x3, y3):
    convert_shape_to_p5(
        clean_identifier(name), f"triangle({x1}, {y1}, {x2}, {y2}, {x3}, {y3})"
    )
...
def convert_quad_to_p5(name, x1, y1, x2, y2, x3, y3, x4, y4):
    convert_shape_to_p5(
        clean_identifier(name), f"quad({x1}, {y1}, {x2}, {y2}, {x3}, {y3}, {x4}, {y4})"
    )
...
def convert_write_to_p5(name, text, x, y):
    convert_shape_to_p5(
        clean_identifier(name), f'text("{text}", {x}, {y})', fill=DEFAULT_STROKE
    )
...
def convert_fill_to_p5(object, r, g, b):
    try:
        ...
        convert_object_new_to_p5(object)
        for shape in CLASSES[clean_identifier(object)]["display()"]:
            shape["fill"] = f"rgb({r}, {g}, {b})"
...
def convert_object_expr_to_p5(function_name, param):
    ...
    match function_name:
        ...
        case "ellipse":
            convert_ellipse_to_p5(
                mock_identifier, param[0], param[1], param[2], param[3]
            )

        case "triangle":
            convert_triangle_to_p5(
                mock_identifier,
                param[0],
                param[1],
                param[2],
...
def convert_moveX_to_p5(name, x, start_frame, end_frame):
    ...
    script = (
        f"translate(new p5.Vector({signX} * (frameCount - {start_frame} + {x}), 0));\n"
    )
    try:
        DRAW_CLASSES[name].append([start_frame, end_frame, script])
    except:
    ...
    end_script = (
        f"translate(new p5.Vector({signX} * ({end_frame} - {start_frame} + {x}), 0));\n"
    )
    try:
        DRAW_CLASSES[name].append([end_frame, MAX_FRAMES, end_script])
    except:
...
def convert_moveY_to_p5(name, y, start_frame, end_frame):
    ...
    script = (
        f"translate(new p5.Vector(0, {signY} * (frameCount - {start_frame} + {y})));\n"
    )
    try:
        DRAW_CLASSES[name].append([start_frame, end_frame, script])
    except:
    ...
    end_script = (
        f"translate(new p5.Vector(0, {signY} * ({end_frame} - {start_frame} + {y})));\n"
    )
    try:
        DRAW_CLASSES[name].append([end_frame, MAX_FRAMES, end_script])
    except:
